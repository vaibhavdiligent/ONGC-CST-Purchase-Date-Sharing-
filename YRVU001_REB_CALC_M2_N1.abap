...
REPORT yrvu001_reb_calc_m2_n1  NO STANDARD PAGE HEADING
               MESSAGE-ID yv01 .
*INCLUDES-------------------------------------------------------------*
*TABLES - SAP TABLES--------------------------------------------------*
TABLES : s925,
...
*CONSTANTS------------------------------------------------------------*
DATA: gd_line TYPE i.
*** CIS 2026-27 rebate maker/checker (L1) - always ON in this program ***
DATA: gv_maker_mode TYPE flag VALUE 'X'.
DATA: gt_stg_office TYPE STANDARD TABLE OF vkbur.   " offices staged (L2 mail)
*SELECTION-SCREEN
...
**** BOC CHARM ID 2000000213 TECHNICAL VIKRAM BAJAJ FUNCTIONAL PRIYANKA SINGH DT 08.03.2021
  IF sy-tcode = 'YRVU015' OR sy-tcode = 'YRVU016' OR sy-tcode = 'YRVU015_A' OR sy-repid = 'YRVU001_REB_CALC_M2_N1'.
**** EOC CHARM ID 2000000213 TECHNICAL VIKRAM BAJAJ FUNCTIONAL PRIYANKA SINGH DT 08.03.2021
    CALL FUNCTION 'REUSE_ALV_LIST_DISPLAY'
      EXPORTING
        i_callback_program       = 'YRVU001_REB_CALC_M2_N1'
...
*EXECUTE BUTTON
    WHEN 'FCOD'.
      SET PF-STATUS 'STANDARD' OF PROGRAM 'SAPLSALV'
                              EXCLUDING itab.
      PERFORM top_of_page.
*CALL BAPI  (maker/checker: L1 stages for approval instead of creating CMR)
      IF gv_maker_mode = 'X'.
        PERFORM stage_all_reb.
      ELSE.
        PERFORM check.
      ENDIF.
...
*EXECUTE SELECT ALL
    WHEN 'SALL'.
      SET PF-STATUS 'STANDARD' OF PROGRAM 'YRVU001_REB_CALC_M2_N1' EXCLUDING 'CALC'.  "MODIFIED BY SHWETA SONI TCS
      LOOP AT i_reb2.
        i_reb2-check = 'X'.
        MODIFY i_reb2.
      ENDLOOP.
...
ENDFORM.
*&---------------------------------------------------------------------*
*&      Form  STAGE_ALL_REB   (CIS 2026-27 rebate maker - L1)
*&---------------------------------------------------------------------*
*   Maker action: instead of creating the credit memo, stage every ticked
*   result row into YCIS_APPRVL (scheme_type 'U', wf_status '20' Pending
*   L2). The L3 executor (YCIS_REB_EXECUTE) creates the CMR after L2/L3
*   approval, mirroring FORM check / FORM sales_order.
*&---------------------------------------------------------------------*
FORM stage_all_reb.
  DATA: lv_cnt  TYPE i,
        lv_skip TYPE i,
  ...
  LOOP AT i_reb2 WHERE check = 'X'.
    IF i_reb2-value IS INITIAL.           " zero rebate -> do not flow to L2
      lv_skip = lv_skip + 1.
      CONTINUE.
    ENDIF.
    PERFORM stage_one_reb CHANGING lv_cnt.
  ENDLOOP.
  IF lv_cnt > 0.
    COMMIT WORK.
    LOOP AT gt_stg_office INTO lv_off.
      PERFORM send_reb_mail USING '2' lv_off.
    ENDLOOP.
    MESSAGE |{ lv_cnt } rebate line(s) submitted for L2 approval (YCIS_APPRVL)| TYPE 'S'.
  ELSEIF lv_skip > 0.
    MESSAGE |{ lv_skip } line(s) not submitted: zero rebate value| TYPE 'I'.
  ELSE.
    MESSAGE 'Please select at least one line to submit for approval' TYPE 'I'.
  ENDIF.
ENDFORM.                    "stage_all_reb
*&---------------------------------------------------------------------*
*&      Form  STAGE_ONE_REB
*&---------------------------------------------------------------------*
FORM stage_one_reb CHANGING p_cnt TYPE i.
  DATA: ls    TYPE ycis_apprvl,
        lv_wf TYPE ycis_apprvl-wf_status.
  CLEAR ls.
  ls-mandt       = sy-mandt.
  ls-qais_no     = '0'.                   " rebate has no CIS number
  ls-scheme_type = 'U'.                   " Upliftment Rebate (YRVU001)
  ls-period_from = s_sptag-low.
  ls-period_to   = s_sptag-high.
  ls-kunnr       = i_reb2-pkunag.
  ls-kvgr2       = i_reb2-kvgr2.
  ls-cust_name   = i_reb2-name1.
*   payload the L3 executor replays into BAPI_SALESDOCU_CREATEFROMDATA
  ls-doc_type    = 'ZP09'.
  ls-sales_org   = s_vkorg-low.
  ls-ord_reason  = 'G21'.
  ls-sales_off   = i_reb2-vkbur.
  ls-material    = 'REBATE(POLYMER)'.
  ls-werks       = i_reb2-werks.
  ls-kukla       = i_reb2-kukla.
  ls-ref_cust    = i_reb2-kunnr.          " consignment stockist (ref_1)
  ls-rebate_val  = i_reb2-value.          " -> cd_value1 = value/10 at L3
  ls-elig_qty    = i_reb2-ummenge1.
  ls-rev_qty     = i_reb2-ummenge1.       " -> YRVA_REBATE-rev_qty
  ls-lft_qty     = i_reb2-ummenge.        " -> YRVA_REBATE-lft_qty
  ls-target_qty  = i_reb2-ummenge1 * 1000." -> order item target_qty
  ls-reb_cond    = p_kschl.               " condition type -> cd_type1 map
  ls-purch_no    = i_reb2-rem.
  ls-waers       = 'INR'.
*   audit / status  (L1 confirm -> pending L2)
  ls-status      = 'P'.
  ls-wf_status   = '20'.
  ls-maker       = sy-uname. ls-maker_date = sy-datum. ls-maker_time = sy-uzeit.
  ls-l1_user     = sy-uname. ls-l1_date   = sy-datum. ls-l1_time   = sy-uzeit.
  ls-remarks     = 'L1 approved'.
*   duplicate / re-execute guard: skip if already forwarded to L2/L3/done
  ...
  COLLECT i_reb2-vkbur INTO gt_stg_office.
  p_cnt = p_cnt + 1.
ENDFORM.                    "stage_one_reb
*&---------------------------------------------------------------------*
*&      Form  SEND_REB_MAIL   (L1 -> L2 notification, reuses YCIS_WF_APPR)
*&---------------------------------------------------------------------*
FORM send_reb_mail USING p_level  TYPE ycis_wlevel
                         p_office  TYPE vkbur.
  ...
  TRY.
      ...
      ls_text-line = |Rebate (PSD) lines have been verified and submitted by L1.|.
      APPEND ls_text TO lt_text.
      ls_text-line = |Please log in to the L2 approval T-Code and complete the approval.|.
      APPEND ls_text TO lt_text.
      ls_text-line = ||.                                             APPEND ls_text TO lt_text.
      ls_text-line = |This is a system generated mail. Please do not reply.|.
      APPEND ls_text TO lt_text.
      lv_sub = 'Rebate (PSD) - lines submitted by L1 for approval'.
