*&---------------------------------------------------------------------*
*& Report  YCIS_REB_APPRV5
*&---------------------------------------------------------------------*
*& Rebate (PSD / YRVU001) - 6-level approval workflow : LEVEL 5
*& (CPC Head - Final Approval).
*&
*&   L4 : YCIS_REB_VET vets the rebate order -> WF_STATUS '50' (Pending L5).
*&        THIS program lists those Pending-L5 rebate rows for the CPC Head:
*&          APPROVE (Final) -> WF_STATUS '60' (Pending L6 - Disbursement), mail L6.
*&          REJECT          -> WF_STATUS '40' (back to L4), mail L4.
*&
*&   Rebate queue only (SCHEME_TYPE = 'U'); L5 is central (office '0001').
...
CONSTANTS: gc_level  TYPE ycis_wlevel VALUE '5',              " this program = L5
...
START-OF-SELECTION.
  ...
  IF gt_appr IS INITIAL.
    MESSAGE 'No rebate records pending your (L5) final approval' TYPE 'I'.
    RETURN.
  ENDIF.
  PERFORM build_out.
...
FORM top_of_page.                                           "#EC CALLED
  ...
  ls_hdr-info = 'Rebate (PSD) - Level-5 Final Approval (CPC Head)'.
...
FORM process_selected USING p_action TYPE char1.
  ...
  LOOP AT gt_out INTO gs_out WHERE sel = 'X'.
    ...
    CHECK sy-subrc = 0.
    IF p_action = 'A'.
      gs_appr-wf_status = '60'.          " Pending L6 (Disbursement)
      gs_appr-status    = 'P'.
      ...
      lv_appr = lv_appr + 1.
    ELSE.
      gs_appr-wf_status   = '40'.        " back to L4
      gs_appr-status      = 'P'.
  ...
  ENDIF.
  IF lv_appr > 0.
    PERFORM send_mail USING 'F' '6'.      " forward to L6
  ENDIF.
  IF lv_rej > 0.
    PERFORM send_mail USING 'R' '4'.      " reject back to L4
  ENDIF.
  DELETE gt_out WHERE sel = 'X'.
  MESSAGE |{ lv_appr } approved & forwarded to L6, { lv_rej } returned to L4| TYPE 'S'.
ENDFORM.
...
FORM send_mail USING p_kind    TYPE char1
  ...
  TRY.
      ...
      IF p_kind = 'F'.
        ls_text-line = |Final approval of the rebate (PSD) orders has been granted by L5 (CPC Head).|.
        APPEND ls_text TO lt_text.
        ls_text-line = |Please log in and process the disbursement (L6 - CPC Finance).|.
        APPEND ls_text TO lt_text.
        lv_sub = 'Rebate (PSD) - Approved by L5, Action Required at L6'.
      ELSE.
        ls_text-line = |The rebate (PSD) orders have been returned by L5 (CPC Head) for your review.|.
        APPEND ls_text TO lt_text.
        ls_text-line = |Please log in to the L4 (CPC Finance) vetting screen and re-check the records.|.
        APPEND ls_text TO lt_text.
        lv_sub = 'Rebate (PSD) - Returned by L5 to L4'.
      ENDIF.
      ls_text-line = ||.                                             APPEND ls_text TO lt_text.
