...
l_tabix = sy-tabix.
SELECT single bukrs into l_bukrs from pa0001 where pernr = wa_pa0009-pernr
AND begda <= p_date and endda >= p_date . "p_date
IF sy-subrc = 0.
* End of <> on 16062011
IF l_bukrs = 'OVL'.
...
* Begin of <> on 231110
SELECT * from zfivms_resend into table ist_zfivms_resend where status = 'DEREGISTRATION FAILURE'.
* End of <>
...
""""""""""
SELECT * from zfivms_brd into table ist_zfivms_brd where ( vmc_apdate EQ p_date and span in s_span )
AND status in ('RELEASE BY VMC' , 'RELEASE BY EMPLOYEE' , 'RELEASE BY VMC FOR OVL', 'RELEASE BY EMPLOYEE FOR OVL').
...
"""""""""
"added by lipsy on 04.01.2013 for file generation even if customer bank data is not there RD1K983162
SELECT * from zsdcust_bankupd into table ist_zsdcust_bankupd where
span in s_span
AND status in ('RELEASE BY CMC').
...
"bank data is not there RD1K983162
""""""""""
SELECT * from zfivms_brd into table ist_zfivms_brd where span in s_span
AND status in ('RELEASE BY VMC' , 'RELEASE BY EMPLOYEE' , 'RELEASE BY VMC FOR OVL', 'RELEASE BY EMPLOYEE FOR OVL').
...
* End of <> on 09052012
*
* status in ('APPROVED BY VMC' , 'APPROVED BY EMPLOYEE' , 'APPROVED BY VMC FOR OVL', 'APPROVED BY EMPLOYEE FOR OVL'). 290911
"commented by lipsy on 04.01.2013 for file generation even if customer
"bank data is not there RD1K983162
* if sy-subrc = 0.
"commented by lipsy on 04.01.2013 for file generation even if customer
...
"added by lipsy on 04.01.2013 for file generation even if customer
"bank data is not there RD1K983162
IF ist_zfivms_brd is not INITIAL or ist_zsdcust_bankupd is NOT INITIAL.
"end of addition by lipsy on 04.01.2013 for file generation even if customer bank data is not there RD1K983162
""""""""
...
LOOP at ist_zfivms_brdt into wa_zfivms_brdt where span = wa_zfivms_brd1-span.
...
"16072012
IF l_line > 0.
READ table ist_zfivms_brdtmp into wa_zfivms_brdtmp with key seqno = l_line.
IF sy-subrc <> 0.
...
MOVE-CORRESPONDING ist_ddata_nonsbi to ist_ddata_nonsbi_ovl_e. "28062011
...
ELSEIF wa_zfivms_brd-status = 'RELEASE BY EMPLOYEE FOR OVL'. "
PERFORM add_7x_emp.
CONCATENATE wa_zfivms_brd-koinh_new '*' wa_zfivms_brd-bankn_new '*' valid_input_as '*' l_spanasa '*' into ist_rdata_sbi-span.
...
LOOP at ist_zfivms_brd into wa_zfivms_brd where status = 'RELEASE BY EMPLOYEE' .
...
"16062011
*
CONCATENATE '/usr/sap/' syst-sysid '/DigSignApp/data/output/signed/mum/' '3P.' g_date_rd '.94427' '.txt' into g_filename1. "04052011
* End of <RD1K976705> on 10062011
*
CONCATENATE '/usr/sap/syst-sysid/DigSignApp/data/output/signed/mum/' '3P.' g_date_rd '.94427' '.txt' into g_filename1. "11042011
*
...
** Opening the File
*
OPEN dataset g_filename1 for output in text mode ENCODING DEFAULT MESSAGE D_MSG_TEXT.
...
* End of <> on 10062011
*
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/mum/' 'D3P.' g_date_rd '.94427' '.txt' into g_filename1. "11042011
*
...
*
CONCATENATE '/usr/sap/' syst-sysid '/DigSignApp/data/output/signed/mum/' 'IBTP.' g_date_rd '.94427' '.txt' into g_filename1. "11042011
* End of <> 10062011
*
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/mum/' 'IBTP.' g_date_rd '.94427' '.txt' into g_filename1."11042011 04052011
*
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/mum/' 'IBTP' g_date_rd g_time_rd '.txt' into g_filename1.
* End of <> on 24212010
...
ENDIF.
*
CONCATENATE '/usr/sap/' syst-sysid '/DigSignApp/data/output/signed/mum/' 'DIBTP.' g_date_rd '.94427' '.txt' into g_filename1. "11042011
* End of <> 10062011
*
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/mum/' 'DIBTP.' g_date_rd '.94427' '.txt' into g_filename1.
"11042011
*
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/mum/' 'DIBTP' g_date_rd g_time_rd '.txt' into g_filename1.
* End of <> on 24122010
**Begin RD1K991769 CAB_ALOK
...
* Begin of Comment on 07082012
*
CONCATENATE '/usr/sap/RP1/FIN/epay/send/mum/' 'DIBTP' g_date_rd g_time_rd '.txt' into g_filename.
...
ENDIF.
*
CONCATENATE '/usr/sap/' syst-sysid '/DigSignApp/data/output/signed/ovl/' '3P.' g_date_rd '.30523' '.txt' into g_filename1.
* End of <> on 10062011
**Begin RD1K991769 CAB_ALOK
...
ENDIF.
*
CONCATENATE '/usr/sap/' syst-sysid '/DigSignApp/data/output/signed/ovl/' 'IBTP.' g_date_rd '.30523' '.txt' into g_filename1.
* End of <> on 10062011
...
ENDIF.
*
CONCATENATE '/usr/sap/' syst-sysid '/DigSignApp/data/output/signed/ovl/' 'DIBTP.' g_date_rd '.30523' '.txt' into g_filename1.
* End of <> on 10062011
**Begin RD1K991769 CAB_ALOK
...
* Begin of Comment on 07082012
*
CONCATENATE 'C:\SPAN\' 'R_SBI_E' g_date_rd g_time_rd '.txt' into g_filename.
...
* Begin of Comment on 07082012
*
CONCATENATE 'C:\SPAN\' 'D_SBI_E' g_date_rd g_time_rd '.txt' into g_filename. "21122010 "11042011 21060211 291211 27012012
*
*
CALL FUNCTION 'GUI_DOWNLOAD'
*
...
* End of <> on 10062011
*
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/ddn/' 'D3P.' g_date_rd '.93128' '.txt' into g_filename1. "11042011
**Begin RD1K991769 CAB_ALOK
...
* End of <> 10062011
*
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/ddn/' 'IBTP.' g_date_rd '.93128' '.txt' into g_filename1. "11042011
**Begin RD1K991769 CAB_ALOK
...
* End of <> on 10062011
*
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/ddn/' 'DIBTP.' g_date_rd '.93128' '.txt' into g_filename1.
"11042011
**Begin RD1K991769 CAB_ALOK
...
** End of <> 10062011
**
CONCATENATE '/usr/sap/RD1/DigSignApp/data/output/signed/ddn/' '3P.' g_date_rd '.93128' '.txt' into g_filename1. "11042011
*
*
*
OPEN dataset g_filename1 for output in text mode ENCODING DEFAULT.
...
TABLES
*
DATA_TAB = ist_rdata_nonsbi_ovl_e
*
EXCEPTIONS
*
FILE_WRITE_ERROR = 1
...
TABLES
*
DATA_TAB = ist_ddata_nonsbi_ovl_e
*
EXCEPTIONS
*
FILE_WRITE_ERROR = 1
...
*
STATUS = wa_zfivms_brd-STATUS
*
REMARKS = wa_zfivms_brd-REMARKS
*
WHERE span = wa_zfivms_brd-span .
ELSE.
* End of <> on 20062011
...
REFRESH ist_pa0006.
SELECT * from pa0006 into table ist_pa0006 where pernr = wa_pa0009t-pernr
AND anssa in ('1', '3'
)
AND endda = l_enddate.
"'31.12.9999'.
...
** New Format
**S D ComBenNSRegSBAdd1 00000037773091731 EDYWYE35 ICIC0000001 MIN NAGAR WEST NETHAJI AV TAMILNADU INDIA
**
...
l_dummy(10),
l_span TYPE zfivms_brd-span, " ZSPAN,
l_street TYPE zfivms_brd-street,
l_str_suppl1 type zfivms_brd-str_suppl1,
l_city1 TYPE zfivms_brd-city1,
l_phone(30).
...
DATA: l_vmc_apdate TYPE zfiben_file_moni-vmc_apdate,
l_sent_date TYPE zfiben_file_moni-sent_date,
l_sent_time TYPE zfiben_file_moni-sent_time,
l_sent_filename type zfiben_file_moni-sent_filename,
l_sent_flag TYPE zfiben_file_moni-sent_flag,
l_sent_userid TYPE zfiben_file_moni-sent_userid,
l_sent_error TYPE zfiben_file_moni-sent_error,
l_raw_data TYPE xstring, "ZFIBEN_FILE_MONI-RAW_DATA,
l_data_sent TYPE zfiben_file_moni-data_sent.
*=============================================
******** Monitoring
l_vmc_apdate = p_date.
...
IF p_payctr = 'CVP'.
* COMPBEN. CURR_DATE 070114.ONGC94427 VMC_APDATE SENT_TIME .00000030340574145.txt
CONCATENATE 'ONGC94427' l_vmc_apdate l_sent_time into corpfilename
.
*
IF sy-host = 'eccdci' OR sy-host = 'eccqci'. "S4H FP3 changes
*
CONCATENATE 'COMPBEN' CURR_DATE CorpFileName '00000030003223534.txt' INTO L_SENT_FILENAME SEPARATED BY '.' . "changes on 30.07.14
CONCATENATE 'COMPBEN' curr_date corpfilename '00000030340574145' into l_sent_filename separated by '.' .
"changes on 30.07.14
*
ELSEIF sy-host+0(6) = 'eccpap'.
"S4H FP3 changes
*
CONCATENATE 'COMPBEN' CURR_DATE CorpFileName '00000030340574145.txt' INTO L_SENT_FILENAME SEPARATED BY '.' . "changes on 30.07.14
*
...
CONCATENATE 'COMPBEN' curr_date corpfilename '00000030717314079' into l_sent_filename separated by '.' .
"changes on 30.07.14
*
ELSEIF sy-host+0(6) = 'eccpap'. "S4H FP3 changes
*
CONCATENATE 'COMPBEN' CURR_DATE CorpFileName '00000030717314079.txt' INTO L_SENT_FILENAME SEPARATED BY '.' . "changes on 30.07.14
*
...
***end trust
IF p_payctr = 'OVL'.
* OVL : COMPBEN.CURR_DATE 070114.OVL30523 VMC_APDATE SENT_TIME . 00000010277786661.txt
CONCATENATE 'OVL30523' l_vmc_apdate l_sent_time into corpfilename .

*
IF sy-host = 'eccdci' or sy-host = 'eccqci'. "S4H FP3 changes
*
CONCATENATE 'COMPBEN' CURR_DATE CorpFileName '00000030003139979.txt' INTO L_SENT_FILENAME SEPARATED BY '.' . "changes on 30.07.14
*
...
e_int TYPE i.
DATA: lo_bnk_detail TYPE ref to zfi_bnk_sbi_in_encco_si_sbiweb,
proxy_data TYPE zfi_bnk_sbi_in_encfile_upload2,
lt_input TYPE zfi_bnk_sbi_in_encfile_upload1,
...
*
TXTDOCUMENT = doc_tab
bindocument = l_raw_data
doc_type    = 'TXT'
*   ssf_id      = 'CN=TEST CLASS IIIB SHATWO, SP=Gujarat, postalCode=392015, OU=CID - 2431135, OU=Marketing, O=Gujarat Narmada Valley Fertilizers Company Limited, C=IN'."wa_ZFI_BATCH_SIGN-ssf_screen.
IMPORTING
crc = crc
signature = doc_sig
EXCEPTIONS
...
ENDIF.
ENDIF.
proxy_data-parameters-arg0 = l_sent_filename.
proxy_data-parameters-arg1 = l_data_sent.
...
ENDTRY.
** update Moniotoring logs in table ZFIBEN_FILE_MONI
...
SELECT * from zsdcust_bank into table lit_zsdcust_bank1 where chon = p_date
AND status = 'RELEASE BY CMC'.
IF sy-subrc eq 0.
LOOP at lit_zsdcust_bank1 into lwa_zsdcust_bank1.
lwa_zsdcust_bank1-status = 'SENT TO SBI'.
MODIFY lit_zsdcust_bank1 from lwa_zsdcust_bank1 index sy-tabix.
ENDLOOP.
...
DATA: it_zfi_alerts type table of ty_zfi_alerts,
wa_zfi_alerts type ty_zfi_alerts,
wa_zfi_alerts1 type ty_zfi_alerts,
wa_zfi_alerts2 type ty_zfi_alerts,
wa_zfi_alerts3 type ty_zfi_alerts,
wa_obj_head TYPE solisti1,
wa_obj_cont TYPE solisti1,
it_obj_cont TYPE table of solisti1,
it_obj_head TYPE table of solisti1,
mail_sub TYPE sodocchgi1,
it_reclist TYPE table of somlreci1,
wa_reclist TYPE somlreci1,
lwa_zfivmsbank type zfivmsbank,
lit_zfivmsbank type table of zfivmsbank,
wa_new1 TYPE zfivmsbank.
...
mob_no(12),
result TYPE string,l_reason_desc(35), l_result(35),
...
CLEAR: wa_zfi_alerts,wa_zfi_alerts1,wa_zfi_alerts2,wa_zfi_alerts3.

"SAPMAIL'

REFRESH:it_obj_cont,it_obj_head,it_reclist.
READ table it_zfi_alerts into wa_zfi_alerts with key categ = 'SAPMAIL'
action = 'SUBJECT'

ident = 'SENT_2_SBI'.
"'SENT_2_SBI' duplicate removed.
IF sy-subrc = 0.

"ident =

REPLACE all occurrences of 'XXXXXXXXXX' in wa_zfi_alerts-matt with lwa_zfivmsbank-reqno.
mail_sub-obj_descr = wa_zfi_alerts-matt.
"= 'SAPMAIL'

READ table it_zfi_alerts into wa_zfi_alerts1 with key categ = 'SAPMAIL'
...
READ table it_zfi_alerts into wa_zfi_alerts3 with key categ = 'SMS'
...
REPLACE all occurrences of 'XXXXXXXXXX' in wa_zfi_alerts3-matt with lwa_zfivmsbank-reqno.
REPLACE all occurrences of 'ZFIVMSBANK-LIFNR' in wa_zfi_alerts3-matt with lwa_zfivmsbank-lifnr.
REPLACE all occurrences of 'ZFIVMSBANK-NAME1' in wa_zfi_alerts3-matt with wa_new1-name1.
REPLACE all occurrences of '&' in wa_zfi_alerts3-matt with 'and'.
