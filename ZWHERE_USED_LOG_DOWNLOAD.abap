...
SELECTION-SCREEN BEGIN OF BLOCK opt WITH FRAME TITLE TEXT-002.
  PARAMETERS: p_pkg    TYPE i DEFAULT 50000  OBLIGATORY,  " Package size per fetch
              p_path   TYPE string LOWER CASE,             " Local file path (leave blank for dialog)
...
*----------------------------------------------------------------------*
* Helper macro to build delimited line
*----------------------------------------------------------------------*
DEFINE _append_field.
  IF lv_line IS INITIAL.
    lv_line = &1.
  ELSE.
    CONCATENATE lv_line p_delim &1 INTO lv_line.
  ENDIF.
END-OF-DEFINITION.
...
START-OF-SELECTION.
  ...
  DO.
    ...
    lv_fetched = lv_fetched + lines( lt_data ).
    ...
    cl_gui_frontend_services=>gui_download(
      EXPORTING
        filename                = lv_file
        filetype                = 'ASC'
        append                  = COND #( WHEN lv_fetched > p_pkg THEN 'X' ELSE ' ' )
        codepage                = '4110'   " UTF-8
        write_bom               = COND #( WHEN lv_fetched > p_pkg THEN ' ' ELSE 'X' )
      CHANGING
        data_tab                = lt_output
      EXCEPTIONS
        file_write_error        = 1
    ...
    IF sy-subrc <> 0.
      ...
      MESSAGE |File write error at record { lv_fetched }. Check path: { lv_file }| TYPE 'E'.
    ENDIF.
  ...
  MESSAGE |Export complete. { lv_fetched } records written to: { lv_file }| TYPE 'S'.
