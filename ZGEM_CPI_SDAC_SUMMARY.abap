*&---------------------------------------------------------------------*
*& Report ZGEM_CPI_SDAC_SUMMARY
*&---------------------------------------------------------------------*
*& SDAC Summary (3.7) - GeM CPI integration.
...
*&   5. Parse response and display as ALV grid (one row per sdacId).
*&
*& Real response shape:
*&   {"sub":..,"aud":..,"iss":..,"data":[
*&     {"date":..,"count":..,"sdacIds":[{"sdacId":..,"invoiceNumber":..}]}
*&   ]}
*&---------------------------------------------------------------------*
REPORT zgem_cpi_sdac_summary.
...
PARAMETERS:
            p_head  TYPE char70 LOWER CASE DEFAULT 'SDAC Summary (3.7)', " ALV list header (editable)
            ...
            p_path  TYPE string LOWER CASE DEFAULT '/http/GEM/Sync/SdacSummary'.
...
*   sdacIds items carry two fields: sdacId + invoiceNumber.
TYPES: BEGIN OF ty_sdac,
         sdacid        TYPE string,   " sdacId
...
TYPES: BEGIN OF ty_data_block,
         date    TYPE string,
         count   TYPE i,
         sdacids TYPE tt_sdac,   " sdacIds array
...
*--- Flat display: one row per sdacId
...
*--- 6a. Flatten to one row per sdacId per date block
  CLEAR lt_display.
  ...
  IF lt_display IS NOT INITIAL.
    TRY.
        ...
        lo_alv->get_display_settings( )->set_list_header(
          |{ ls_response-sub } - { ls_response-aud } - { ls_response-iss } - { lines( lt_display ) } SDAC(s)| ).
        lo_alv->display( ).
