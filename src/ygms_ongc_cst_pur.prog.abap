...
*& Version: 5.0 - Full featured with mapping and validation
*&---------------------------------------------------------------------*
REPORT ygms_ongc_cst_pur.
...
FORM process_upload.
  ...
  " Step 5: Fetch GCV/NCV values
  PERFORM fetch_gcv_ncv.
  CHECK gv_errors = 0.

  " Step 6: Check existing purchase data
  PERFORM check_existing_purchase_data.
...
*&---------------------------------------------------------------------*
FORM process_view.
  DATA: lt_view_data TYPE TABLE OF ty_upload_data.

  " Build WHERE clause dynamically
  SELECT gas_day, location_id, ctp_id, material, ongc_material,
         qty_scm, gcv, ncv
    FROM yrga_cst_b2b_1
    INTO CORRESPONDING FIELDS OF TABLE @lt_view_data
    WHERE ( @p_ctp IS INITIAL OR ctp_id = @p_ctp )
      AND ( @s_gasday IS INITIAL OR gas_day IN @s_gasday )
      AND ( @p_mat IS INITIAL OR ongc_material = @p_mat ).

  IF lt_view_data IS INITIAL.
    MESSAGE s000(ygms_msg) WITH 'No data found for selection criteria'.
    RETURN.
  ENDIF.

  gt_upload_data = lt_view_data.
  PERFORM display_alv.
ENDFORM.
  ...
  LOOP AT gt_alsmex_data INTO DATA(ls_alsmex).
    ...
    TRY.
        CASE ls_alsmex-col.
          ...
          WHEN 4.
            DATA(lv_qty) = ls_alsmex-value.
            REPLACE ALL OCCURRENCES OF ',' IN lv_qty WITH '.'.
            ls_excel-qty_scm = lv_qty.
        ENDCASE.
      CATCH cx_root.
...
FORM validate_fortnight.
  DATA: lv_min_date   TYPE datum,
        ...
        lv_fn2_start  TYPE datum,
        lv_fn2_end    TYPE datum.
  ...
  DATA(lv_day) = lv_min_date+6(2).

  IF lv_day <= '15'.
    " First fortnight
    lv_fn1_start = lv_min_date(6) && '01'.
    lv_fn1_end   = lv_min_date(6) && '15'.
  ELSE.
    " Second fortnight
    lv_fn1_start = lv_min_date(6) && '16'.
    " Get last day of month
    DATA(lv_next_month) = lv_min_date.
...
FORM map_location_to_ctp.
  ...
  SELECT * FROM yrga_cst_loc_map
    ...
    WHERE gail_loc_id IN @(
      VALUE rseloption( FOR loc IN lt_locations
        ( sign = 'I' option = 'EQ' low = loc ) ) )
  ...
  LOOP AT gt_excel_data INTO ls_excel.
    DATA(ls_upload) = CORRESPONDING ty_upload_data( ls_excel ).
...
FORM map_material_to_ongc.
  ...
  " Get unique location-material combinations
...
FORM fetch_gcv_ncv.
  ...
  LOOP AT gt_upload_data ASSIGNING FIELD-SYMBOL(<fs_upload>).
    ...
    SELECT SINGLE gcv, ncv
      FROM yrxa_cmdata
      INTO (@<fs_upload>-gcv, @<fs_upload>-ncv)
      WHERE location_id = @<fs_upload>-location_id
        AND gas_day     = @<fs_upload>-gas_day.

    IF sy-subrc <> 0.
      MESSAGE e001(ygms_msg) WITH 'GCV/NCV not found for' <fs_upload>-location_id <fs_upload>-gas_day.
      lv_error = abap_true.
      gv_errors = gv_errors + 1.
      CONTINUE.
    ENDIF.

    IF <fs_upload>-gcv <= 0 OR <fs_upload>-ncv <= 0.
      MESSAGE e001(ygms_msg) WITH 'Invalid GCV/NCV for' <fs_upload>-location_id <fs_upload>-gas_day.
      lv_error = abap_true.
      gv_errors = gv_errors + 1.
    ENDIF.
  ENDLOOP.
...
*&---------------------------------------------------------------------*
*& Form CHECK_EXISTING_PURCHASE_DATA
*&---------------------------------------------------------------------*
FORM check_existing_purchase_data.
  ...
  SELECT COUNT(*)
    FROM ygms_cst_pur
    INTO @lv_count
    WHERE ctp_id IN @( VALUE rseloption( FOR ctp IN lt_ctp_ids
            ( sign = 'I' option = 'EQ' low = ctp ) ) )
  ...
  IF lv_count > 0.
    IF gv_b2b_mode = abap_false.
      ...
      CALL FUNCTION 'POPUP_TO_CONFIRM'
        EXPORTING
          titlebar              = 'Warning'
          text_question         = 'Purchase data exists for this fortnight. Delete and proceed?'
          text_button_1         = 'Yes'
          text_button_2         = 'No'
          default_button        = '2'
    ...
    PERFORM delete_existing_data.
  ENDIF.
ENDFORM.

*&---------------------------------------------------------------------*
*& Form DELETE_EXISTING_DATA
*&---------------------------------------------------------------------*
FORM delete_existing_data.
  DATA: lt_ctp_ids TYPE TABLE OF ygms_de_ongc_ctp.
  ...
  DELETE FROM ygms_cst_pur
    WHERE ctp_id IN ( SELECT ctp FROM @lt_ctp_ids AS t(ctp) )
      AND gas_day BETWEEN @gv_fn_start AND @gv_fn_end.

  " Delete from YGMS_CST_FNT (formerly YGMS_CST_FNT_DATA)
  DELETE FROM ygms_cst_fnt
    WHERE ctp_id IN ( SELECT ctp FROM @lt_ctp_ids AS t(ctp) )
      AND fn_start = @gv_fn_start
      AND fn_end   = @gv_fn_end.

  COMMIT WORK AND WAIT.

  MESSAGE s000(ygms_msg) WITH 'Existing purchase data deleted.'.
ENDFORM.
