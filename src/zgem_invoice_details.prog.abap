...
*& are hard-coded. Fetched rows are saved to ZGEM_INVDET and shown in ALV.
*&---------------------------------------------------------------------*
REPORT zgem_invoice_details.
...
DATA: lo_client   TYPE REF TO if_http_client,
      ...
      lv_item     TYPE posnr,
      it_out      TYPE STANDARD TABLE OF zgem_invdet,
      wa_out      TYPE zgem_invdet,
  ...
  LOOP AT ls_response-data INTO ls_inv.
    ...
    LOOP AT ls_inv-consignmentdetails INTO ls_cons.
      LOOP AT ls_cons-products INTO ls_prod.
        ...
        MODIFY zgem_invdet FROM wa_out.
