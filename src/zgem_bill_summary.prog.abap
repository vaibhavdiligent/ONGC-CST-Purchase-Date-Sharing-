...
*&---------------------------------------------------------------------*
*& Bill Summary - GeM CPI integration (customer version).
*& Selection screen: from-date / to-date only. All other request fields
*& are hard-coded. Fetched rows are saved to ZGEMC_BILLSUMM and shown in ALV.
*&
*& Real response shape (confirmed):
*&   {"sub":..,"aud":..,"iss":..,"data":[
*&     {"date":..,"count":..,"gemInvoiceNos":[{"gemInvoiceNo":..}]}]}
*&---------------------------------------------------------------------*
REPORT zgem_bill_summary.
...
DATA: lo_client   TYPE REF TO if_http_client,
      ...
      it_out      TYPE STANDARD TABLE OF zgemc_billsumm,
      wa_out      TYPE zgemc_billsumm,
...
*--- Flatten to one row per gemInvoiceNo, save to ZGEMC_BILLSUMM, collect for ALV
  CLEAR it_out.
  LOOP AT ls_response-data INTO ls_block.
    LOOP AT ls_block-geminvoicenos INTO ls_inv.
      CLEAR wa_out.
      wa_out-gem_invoice_no = ls_inv-geminvoiceno.
      ...
      MODIFY zgemc_billsumm FROM wa_out.
  ...
  IF it_out IS NOT INITIAL.
    TRY.
        ...
        lo_alv->get_display_settings( )->set_list_header(
          |GeM Bill Summary - { lines( it_out ) } invoice(s)| ).
        lo_alv->display( ).
