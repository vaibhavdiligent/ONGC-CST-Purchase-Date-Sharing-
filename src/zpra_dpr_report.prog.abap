...
*&           Fix GRAND-TOTAL hyphen bug (silent ASSIGN COMPONENT failure) in
*&           sec6 Production Performance table: fill_dynamic_table_sec6a,
*&           get_annual_targets_6b, get_ytd_targets_6b now use lv_col_name
*&           variable with CONCATENATE 'GRAND' '-' 'TOTAL' instead of literal.
*&           This fixes 0 values for Total BOEPD Annual/YTD columns.
*&           v2.3 - Skip Priority 3b (ZPRA_DLY_PRD_ND) for Vietnam in Section 3F:
...
*&           v2.1 - Fix Production Performance table and chart (section 6/5a):
...
*& Row numbers are resolved dynamically from the section start-row
*& globals so the formulas stay correct regardless of DPR layout shifts.
*&---------------------------------------------------------------------*
FORM write_sec6_formulas USING p_actual_row .
  ...
  " Current name of the DPR worksheet (still '1' here; finalize_worksheet
  " renames it to 'DPR ( <date> )' later and Excel auto-updates these refs).
  GET PROPERTY OF go_worksheet 'Name' = lv_dpr_name .
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form  SET_SHEET3_FORMULA
*& Writes ='<sheet>'!<col><row> into a single cell of sheet 3 (go_worksheet3)
*&---------------------------------------------------------------------*
FORM set_sheet3_formula USING p_row p_col p_sheet p_colltr p_rownum .
  DATA : lv_formula TYPE string,
         lo_cell    TYPE ole2_object.
  " Build ='<sheet name>'!<COL><ROW>  (single quotes around the sheet name)
...
FORM fill_dynamic_table_sec5a .
  DATA : lv_date           TYPE sy-datum,
         ...
         lv_act_prod       TYPE p LENGTH 16 DECIMALS 7,
         lv_index          TYPE i,
         lv_col_name       TYPE lvc_fname.

  " Build component name via variable - literal 'GRAND-TOTAL' fails in ASSIGN COMPONENT
  " because ABAP treats the hyphen as a path separator
  CONCATENATE 'GRAND' '-' 'TOTAL' INTO lv_col_name.

  " Read flat BE target (F48 = Total BOEPD of BE row) from sec2b INDEX 1
  READ TABLE <gfs_sec2b_table> ASSIGNING <gfs_dyn_line2> INDEX 1.
  IF sy-subrc IS INITIAL.
    ASSIGN COMPONENT lv_col_name OF STRUCTURE <gfs_dyn_line2> TO <gfs_field2>.
    IF <gfs_field2> IS ASSIGNED.
      lv_grand_total_be = <gfs_field2>.
      UNASSIGN <gfs_field2>.
    ENDIF.
  ENDIF.
  ...
  DO.
    ...
    " ACT_PRODUCTION = col AG (GRAND-TOTAL) for this date in main DPR table
    IF lv_index GT 0.
      READ TABLE <gfs_dyn_table> ASSIGNING <gfs_dyn_line2> INDEX lv_index.
      IF sy-subrc IS INITIAL.
        ASSIGN COMPONENT lv_col_name OF STRUCTURE <gfs_dyn_line2> TO <gfs_field2>.
        IF <gfs_field2> IS ASSIGNED.
          lv_act_prod = <gfs_field2>.
          ASSIGN COMPONENT 2 OF STRUCTURE <gfs_dyn_line> TO <gfs_field>.
          IF <gfs_field> IS ASSIGNED.
            <gfs_field> = lv_act_prod.
            UNASSIGN <gfs_field>.
          ENDIF.
          UNASSIGN <gfs_field2>.
        ENDIF.
    ...
    IF p_t_be IS NOT INITIAL.
      ASSIGN COMPONENT 'TAR_BE' OF STRUCTURE <gfs_dyn_line> TO <gfs_field>.
      IF <gfs_field> IS ASSIGNED.
        <gfs_field> = lv_grand_total_be.
        UNASSIGN <gfs_field>.
      ENDIF.
    ENDIF.
