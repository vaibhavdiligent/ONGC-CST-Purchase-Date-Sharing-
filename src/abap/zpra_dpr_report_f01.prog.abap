...
FORM prepare_dynamic_table_sec1 .
  ...
  gv_table_columns = gv_pos .>P_COLPOS  text>P_COL01  text>P_COLTEXT  text>P_OUTPUTLEN  text
...
*  CHECK lv_lines IS NOT INITIAL .
  IF lv_lines IS INITIAL.
    lv_lines = 1.
  ENDIF.
*End of changes by AbhishekTR OCDK904738Changes to show blank line if target data is not availa
  gv_row = gv_row + 1 .
...
*  CHECK lv_lines IS NOT INITIAL .
  IF lv_lines IS INITIAL.
    lv_lines = 1.
  ENDIF.
*Changes to show blank line if target data is not availaEnd of changes by AbhishekTR OCDK904738
  gv_row   = gv_row + 1 .
  gv_s_row = gv_row .
  gv_s_col = 1 .
  gv_e_row = gv_s_row + lv_lines - 1 .
...
*  CHECK lv_lines IS NOT INITIAL .
  IF lv_lines IS INITIAL.
    lv_lines = 1.
  ENDIF.
*Changes to show blank line if asking rate is not availaEnd of changes by AbhishekTR OCDK904754
  gv_row = gv_row + 1 .
  gv_s_row = gv_row .
  gv_s_col = 1 .
  gv_e_row = gv_s_row + lv_lines - 1 .
...
*ble
  IF gt_paste IS NOT INITIAL.
    PERFORM paste_data .
  ENDIF.
*End of changes by AbhishekTR OCDK904754Changes to show blank line if asking rate is not availa
...
FORM display_section6 .
  ...
  CONCATENATE 'Total (O+OEG) (' lv_gt_unit ')' INTO gv_txt SEPARATED BYspace.
  PERFORM select_range USING gv_s_row gv_s_col gv_e_row gv_e_col  .
  CALL METHOD OF go_range 'Merge' .
  PERFORM set_range USING gv_txt 0.
...
*       text
*
*>P_GV_ROW  text>P_GV_COL  text>P_0393   text>P_GV_ROW  text>P_1      text>P_GV_SEC1_LINES  text>P_30     text
*
...
FORM set_range_formatting USING p_wraptext
                                p_horizontal>P_1263   text>P_1      text>P_1      text>P_1      text>P_1      text
                                p_vertical .
  SET PROPERTY OF go_range 'WrapText' = p_wraptext .
...
FORM row_height  USING  p_row
                        p_height .
  CALL METHOD OF go_excel 'ROWS' = go_row>P_7      text>P_15     text
    EXPORTING
      #1 = p_row .
...
ENDFORM.
*&
*&      Form  ADD_PRODUCT_RANGE>P_0704   text>P_0688   text
*&
*       text
*
*
...
*
*
FORM add_tar_code_range  USING  p_tar_code .>P_0704   text
...
* Here we are calculating weighted PI for each asset for each day. Thiswill be used to convert quantities to OVL level
* Formula used is wtd pi = sum of all block of assets' pi * quantity  divided by sum of all blocks quantity
  ...
  SORT lt_dly_prd BY production_date DESCENDING product ASCENDING assetASCENDING block ASCENDING .
...
****    ENDIF.
    IF ( ls_dly_prd-product         EQ ls_dly_prd_temp-productAND
         ls_dly_prd-asset           EQ ls_dly_prd_temp-assetAND
         ls_dly_prd-production_date EQ  ls_dly_prd_temp-production_dateAND
         ls_dly_prd-block           NE ls_dly_prd_temp-block )
        OR lv_index = 1 .
      IF ls_dly_prd-prod_vl_qty1 IS NOT INITIAL .
        CLEAR lv_pi .
        LOOP AT gt_zpra_t_prd_pi INTO gs_zpra_t_prd_pi WHERE asset   EQls_dly_prd-asset
                                                         AND block   EQls_dly_prd-block
                                                         AND vld_frm LEls_dly_prd-production_date
                                                         AND vld_to  GEls_dly_prd-production_date .
...
FORM fill_null_values_with_previous .
  ...
  DO .
    ...
    IF sy-subrc IS INITIAL.
      SPLIT gs_dyn_fcat-fieldname AT '-' INTO ls_zpra_t_dly_prd-productls_zpra_t_dly_prd-asset .
  ...
  LOOP AT gt_dyn_fcat INTO gs_dyn_fcat .
    ...
    ELSEIF gs_dyn_fcat-fieldname CS 'TOTAL' .
      ...
      CASE lv_product .
        ...
        WHEN OTHERS.
      ENDCASE.
      CONCATENATE 'Total' lv_asset_desc INTO lv_asset_desc SEPARATED BYspace.
    ELSE .
      SPLIT gs_dyn_fcat-fieldname AT '-' INTO lv_product lv_asset .
...
FORM display_units .
  ...
  CASE abap_true.
    ...
    WHEN p_tmd.
      lv_gt_unit   = 'TOEPD' .
    WHEN p_mb.
      lv_gt_unit   = 'MMTOE' .>P_PRODUCT  textP_IND_UNIT  textP_TOTAL_UNIT  text
    WHEN p_bmd.
      lv_gt_unit   = 'BOEPD' .
    WHEN OTHERS.
  ENDCASE.
...
FORM set_section1_header_colors .
  ...
  LOOP AT gt_product_col INTO gs_product_col.
    PERFORM select_range USING gv_start_row gs_product_col-s_col gv_rowgs_product_col-e_col .
...
* First Convert to MCM
  ...
  CASE p_zpra_t_dly_prd-prod_vl_uom1 .
    WHEN 'MCM'.
      lv_qty = p_zpra_t_dly_prd-prod_vl_qty1.P_ZPRA_T_DLY_PRD  text
    WHEN 'M3'.
...
* Convert to display UoM
  CASE abap_true.
    ...
    WHEN p_bmd.
      p_zpra_t_dly_prd-prod_vl_qty1 = lv_qty .              "* 6290.
    WHEN OTHERS.
  ENDCASE.P_ZPRA_T_DLY_PRD  textP_ZPRA_T_DLY_PRD  text
ENDFORM.
*&
...
*      <
*
FORM convert_gas_units_to_mmscm  CHANGING p_zpra_t_dly_prd TYPE zpra_t_dP_ZPRA_T_DLY_PRD  textP_ZPRA_T_DLY_PRD  textP_ZPRA_T_MREC_PRD  textly_prd.
  DATA : lv_qty TYPE char50 .
* First Convert to MCM
...
*&P_ZPRA_T_MREC_PRD  textP_ZPRA_T_MREC_PRD  text
*       text
*
*      <
*
...
FORM convert_non_gas_units  CHANGING p_zpra_t_dly_prd TYPE zpra_t_dly_prd.
  ...
  CASE abap_true.
    WHEN p_bb.
    WHEN p_bbd.
    WHEN p_tm.P_GS_ZPRA_T_DLY_PRD  textP_GS_ZPRA_T_DLY_PRD  text
    ...
    WHEN p_mb .
      ...
      ELSE.
        PERFORM get_cf_from_date USING  p_zpra_t_dly_prd-production_date
                                        ...
                                        p_zpra_t_dly_prd-block
                               CHANGING lv_cf.
      ENDIF.P_GS_ZPRA_T_DLY_PRD  text
      IF lv_cf IS NOT INITIAL.
        p_zpra_t_dly_prd-prod_vl_qty1 = p_zpra_t_dly_prd-prod_vl_qty1 /lv_cf .
*      p_zpra_t_dly_prd-prod_vl_qty1 = p_zpra_t_dly_prd-prod_vl_qty1 / 1000000 .
      ENDIF.
...
FORM convert_non_gas_units3c  USING p_monat TYPE zpra_t_mrec_prd-monat CHANGING p_zpra_t_dly_prd TYPE zpra_t_dly_prd .
  ...
  LOOP AT gt_zpra_t_mrec_prd_3c INTO gs_zpra_t_mrec_prd WHERE product EQ p_zpra_t_dly_prd-product
    ...
    IF gs_zpra_t_mrec_prd-gjahr EQ lv_gjahr AND
       gs_zpra_t_mrec_prd-monat GT lv_monat .
      CLEAR gs_zpra_t_mrec_prd .>P_ZPRA_T_DLY_RPRD  text
      CONTINUE .
    ENDIF.
    EXIT .
  ENDLOOP .
...
***      ELSE.
    p_zpra_t_dly_rprd-jv_prd_vl_qty1 = p_zpra_t_dly_rprd-ovl_rcn_vl_qty3   .
    p_zpra_t_dly_rprd-ovl_prd_vl_qty1 = p_zpra_t_dly_rprd-ovl_rcn_vl_qty
.
***      ENDIF.
*    WHEN OTHERS.
*  ENDCASE.
...
*    ELSE.
*      p_zpra_t_dly_rprd-jv_prd_vl_qty1 = p_zpra_t_dly_rprd-ovl_rcn_vl_qty3 .
*      p_zpra_t_dly_rprd-ovl_prd_vl_qty1 = p_zpra_t_dly_rprd-ovl_rcn_vl_qty3 .>P_ZPRA_T_DLY_RPRD  text
*    ENDIF.
*   WHEN OTHERS.
* ENDCASE.
...
FORM convert_non_gas_units3f  USING p_days CHANGING p_zpra_t_dly_prd TYPE zpra_t_dly_prd .
  DATA : lv_cf TYPE zpra_t_tar_cf-conv_factor .P_GS_ZPRA_T_DLY_PRD  text
...
FORM join_header_total_cells .
  ...
  LOOP AT gt_product_col INTO gs_product_col.
    PERFORM select_range USING gv_sec1_h_start_row gs_product_col-e_collv_end_row gs_product_col-e_col .
    CALL METHOD OF go_range 'Merge' .
  ENDLOOP.
* Grand Total
...
FORM process_sec2a_data .
  ...
  CONCATENATE 'Target' ':' gv_month_name gv_current_calendar_gjahr INTOgv_txt SEPARATED BY space.
...
*       text
*
*   >  p1        textP_<GFS_FIELD>  text>P_GS_ZPRA_T_PRD_TAR_TAR_CODE  text
*  <   p2        text
*
FORM prepare_dynamic_table_sec2e .
  CALL METHOD cl_alv_table_create=>create_dynamic_table
...
FORM get_target_from_name  USING    p_target_name
  ...
  CASE p_target_name.
    ...
    WHEN 'IN'.
      p_tar_code = 'TAR_IN' .
    WHEN 'MOU-Ex'.>P_GS_ZPRA_T_PRD_TAR_TAR_CODE  textP_<GFS_FIELD>  text
      p_tar_code = 'TAR_EX' .
    WHEN 'MOU VG'.
      p_tar_code = 'TAR_VG' .
    WHEN 'PC'.
...
*  CHECK lv_lines IS NOT INITIAL .
  IF lv_lines IS INITIAL.
    lv_lines = 1.
  ENDIF.
*End of changes by AbhishekChanges to show blank line if target data is not availaTR OCDK904738
  gv_row = gv_row + 1 .
...
*&>P_LV_DAYS  text>P_LV_DAYS  textP_GS_ZPRA_T_DLY_PRD  textP_GS_ZPRA_T_DLY_PRD  text
*       text
*
*
*      <
...
FORM finalize_worksheet .
  ...
  CONCATENATE 'DPR (' lv_sheet_name ')' INTO lv_sheet_name SEPARATED BYspace .
...
**            PERFORM get_days_in_period USING gs_zpra_t_mrec_prd-gjahrgs_zpra_t_mrec_prd-monat CHANGING lv_days .
...
**            lv_days = 1 .
**            PERFORM convert_non_gas_mrec_units USING lv_days CHANGINGgs_zpra_t_mrec_prd.
**            IF gs_zpra_t_mrec_prd-product = c_prod_gas.
**             READ TABLE gt_zdpr_gas_combine INTO gs_zdpr_gas_combine WITH KEY asset = gs_zpra_t_mrec_prd-asset BINARY SEARCH .
**             IF  sy-subrc IS INITIAL.
**               lv_combine_field = 'X' .
...
**           ENDIF.
        ELSE.
          ...
          ENDIF.
          READ TABLE gt_zpra_t_dly_rprd_2d INTO gs_zpra_t_dly_rprd WITHKEY product = gs_zpra_c_prd_prof-product
      asset = gs_zpra_c_prd_prof-asset
      block = gs_zpra_c_prd_prof-block
                                                                    production_date = gv_search_begin_datum BINARY SEARCH .
          IF sy-subrc IS NOT INITIAL.
            LOOP AT gt_zpra_t_dly_rprd_2d INTO gs_zpra_t_dly_rprd WHEREproduct         EQ gs_zpra_c_prd_prof-product
                                                                    ANDasset           EQ gs_zpra_c_prd_prof-asset
                                                                    ANDblock           EQ gs_zpra_c_prd_prof-block
                                                                    ANDproduction_date GE gv_search_begin_datum
                                                                    ANDproduction_date LE gv_search_end_datum .
              EXIT .
            ENDLOOP .
          ENDIF.
          IF sy-subrc IS INITIAL.
            ...
            LOOP AT gt_zpra_t_dly_rprd_2d INTO gs_zpra_t_dly_rprd FROM lv_rprd_index .
              ...
              IF gs_zpra_t_dly_rprd-product = c_prod_gas .
                READ TABLE gt_zdpr_gas_combine INTO gs_zdpr_gas_combineWITH KEY asset = gs_zpra_t_dly_rprd-asset BINARY SEARCH .
                IF  sy-subrc IS INITIAL.
                  lv_combine_field = 'X' .
                ENDIF.
              ENDIF.
...
*                  lv_mmscmd_mul = 6290 .
            ...
            ENDLOOP.
          ELSE. "Now work out from daily production
            PERFORM get_days_in_period USING lv_gjahr lv_monat CHANGINGlv_days .
            lv_days = 1 .
            IF lv_monat EQ gv_current_monat.
              CONCATENATE gv_search_end_datum(4) p_date+4(4) INTO gv_search_end_datum .
            ENDIF.
            LOOP AT gt_zpra_t_dly_prd_2d INTO gs_zpra_t_dly_prd WHERE product EQ gs_zpra_c_prd_prof-product
              ...
              IF gs_zpra_t_dly_prd-product = c_prod_gas.
                READ TABLE gt_zdpr_gas_combine INTO gs_zdpr_gas_combineWITH KEY asset = gs_zpra_t_dly_prd-asset BINARY SEARCH .
                IF  sy-subrc IS INITIAL.
                  lv_combine_field = 'X' .
                ENDIF.
              ENDIF.
...
*     <gfs_field> = 'Last Year' .
  LOOP AT gt_zpra_c_prd_prof INTO gs_zpra_c_prd_prof.
    ...
    DO .
      lv_monat = lv_monat + 1 .
      CLEAR lv_combine_field .
      READ TABLE gt_zpra_t_mrec_app_2f INTO gs_zpra_t_mrec_app WITH KEYgjahr   = lv_gjahr
monat   = lv_monat
product = gs_zpra_c_prd_prof-product
asset   = gs_zpra_c_prd_prof-asset
block   = gs_zpra_c_prd_prof-block BINARY SEARCH .
...
*     Grand Total
        ...
        ELSE. "Now work out from daily reconciled production
          PERFORM get_days_in_period USING lv_gjahr lv_monat CHANGING lv_days .
          CLEAR lv_found .
          READ TABLE gt_zpra_t_dly_rprd_3f INTO gs_zpra_t_dly_rprd WITHKEY product = gs_zpra_c_prd_prof-product
      asset = gs_zpra_c_prd_prof-asset
      block = gs_zpra_c_prd_prof-block
                                                                    production_date = gv_search_begin_datum BINARY SEARCH.
          IF sy-subrc IS NOT INITIAL.
            LOOP AT gt_zpra_t_dly_rprd_3f INTO gs_zpra_t_dly_rprd WHEREproduct EQ gs_zpra_c_prd_prof-product
                                                                  ANDasset   EQ gs_zpra_c_prd_prof-asset
                                                                  ANDblock   EQ gs_zpra_c_prd_prof-block
                                                                  AND production_date GE gv_search_begin_datum
                                                                  AND production_date LE gv_search_end_datum .
...
FORM fill_dynamic_table_sec4a .
  ...
  LOOP AT gt_zpra_c_prd_prof INTO gs_zpra_c_prd_prof WHERE product EQ '722000004' .
    ...
    DO .
      lv_monat = lv_monat + 1 .
      READ TABLE gt_zpra_t_mrec_app_4a INTO gs_zpra_t_mrec_app WITH KEYgjahr   = lv_gjahr
monat   = lv_monat
product = gs_zpra_c_prd_prof-product
asset   = gs_zpra_c_prd_prof-asset
block   = gs_zpra_c_prd_prof-block BINARY SEARCH .
...
*     I_MONMIT       = 00
    ...
    EXCEPTIONS
      input_false    = 1
      t009_notfound  = 2>P_GS_ZPRA_T_MREC_PRD_GJAHR  text>P_GS_ZPRA_T_MREC_PRD_MONAT  textP_LV_DAYS  text
      t009b_notfound = 3
      OTHERS         = 4.
  IF sy-subrc <> 0.
    MESSAGE 'Internal Error' TYPE 'E' .
    ...
    IF <gfs_field> IS ASSIGNED.
      LOOP AT gt_zpra_t_dly_prd_5a INTO gs_zpra_t_dly_prd WHERE production_date EQ lv_date.
        ...
        IF p_c_jv IS INITIAL.
          LOOP AT gt_zpra_t_prd_pi_5a INTO gs_zpra_t_prd_pi WHERE assetEQ gs_zpra_t_dly_prd-asset
                                                              AND blockEQ gs_zpra_t_dly_prd-block
...
FORM set_border_range  USING    p_left
  ...
  IF p_bottom EQ 1 .
    CALL METHOD OF go_range 'Borders' = go_border EXPORTING #1 = '9'.
    SET PROPERTY OF go_border 'LineStyle' = '1' .
  ENDIF.>P_1      text>P_1      text>P_1      text>P_1      text>P_1      text>P_1      text>P_1      text>P_1      text
ENDFORM.
*&
...
FORM add_dummy_data  TABLES   pt_zpra_t_dly_prd STRUCTURE zpra_t_dly_prd.
  ...
  DO .
    ...
    IF sy-subrc IS NOT INITIAL.
      ls_zpra_t_dly_prd-mandt           = sy-mandt .>P_LT_ZPRA_T_DLY_PRD  textP_NUMBER_FORMAT  text>P_LV_INDEX  textP_LV_START_DATE  text
      ls_zpra_t_dly_prd-production_date = lv_date .
...
FORM get_target_start_date  USING    p_index
  ...
  DATA :  lv_product TYPE zpra_t_dly_prd-product,
          lv_asset   TYPE zpra_t_prd_pi-asset.P_LV_START_DATE  text>P_LV_INDEX  text
...
FORM populate_no_data_entries  TABLES p_zpra_t_dly_prd STRUCTURE zpra_t_dly_prd
  ...
  DATA : lv_date  TYPE                   sy-datum,
         lv_date2 TYPE                   sy-datum,
         lv_index TYPE                   sy-tabix.>P_GT_ZPRA_T_DLY_PRD  text

  lt_zpra_c_prd_prof = gt_zpra_c_prd_prof .
  SORT lt_zpra_c_prd_prof BY product asset block .
  DELETE ADJACENT DUPLICATES FROM  lt_zpra_c_prd_prof COMPARING productasset .
...
FORM remove_expired_blocks  TABLES   p_zpra_c_prd_prof STRUCTURE gs_zpra_c_prd_prof
  ...
  SELECT asset
         ...
         liscense_exp_dt
    FROM zpra_t_prd_pi
    INTO TABLE gt_prod_start_end_dates>P_GT_ZPRA_C_PRD_PROF  text>P_SY_DATUM  text>P_SY_DATUM  text
...
FORM get_cf_from_date  USING    p_rec_date
  ...
  DATA : lv_monat TYPE t009b-poper,
         lv_gjahr TYPE t009b-bdatj.
>P_P_ZPRA_T_DLY_PRD_PRODUCTION_DA  textP_GS_ZPRA_T_DLY_RPRD  textP_LV_CF  text
  CALL FUNCTION 'DATE_TO_PERIOD_CONVERT'
    EXPORTING
      i_date         = p_rec_date
    " I_MONMIT       = 00
...
ENDFORM.
*&
*&      Form  POPULATE_NO_DATA_ENTRIES_3F>P_GT_ZPRA_T_DLY_PRD_3F  text>P_R_PRODUCTION_DATE_LOW  text>P_R_PRODUCTION_DATE_HIGH  text
*&
*       text
*
*
