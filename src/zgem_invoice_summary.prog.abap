...
CONSTANTS: c_dest TYPE rfcdest VALUE 'CPI_HTTP_GEM',
           c_path TYPE string   VALUE '/http/GEM/Sync/InvoiceSummary'.
...
TYPES: BEGIN OF ty_data_block,
         date           TYPE string,
         count          TYPE i,
         invoicenumbers TYPE tt_invoice,
...
START-OF-SELECTION.
  ...
  LOOP AT ls_response-data INTO ls_block.
    LOOP AT ls_block-invoicenumbers INTO ls_inv.
      CLEAR wa_out.
      wa_out-invoice_number = ls_inv-invoicenumber.
  ...
  IF it_out IS NOT INITIAL.
    TRY.
        ...
        lo_alv->get_display_settings( )->set_list_header(
          |GeM Invoice Summary - { lines( it_out ) } row(s)| ).
        lo_alv->display( ).
