...
*& are hard-coded. Fetched rows are saved to ZGEM_ORDERDET and shown in ALV.
*&---------------------------------------------------------------------*
REPORT zgem_order_details.
...
DATA: lo_client   TYPE REF TO if_http_client,
      ...
      lv_item     TYPE posnr,
      it_out      TYPE STANDARD TABLE OF zgem_orderdet,
      wa_out      TYPE zgem_orderdet,
...
*--- 5. Flatten to one row per product, save to ZGEM_ORDERDET, collect for ALV
  CLEAR it_out.
  LOOP AT ls_response-data INTO ls_order.
    ...
    LOOP AT ls_order-consignmentdetails INTO ls_cons.
      LOOP AT ls_cons-products INTO ls_prod.
        ...
        MODIFY zgem_orderdet FROM wa_out.
